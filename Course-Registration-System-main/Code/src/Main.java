...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (menuChoice != 0) {
            ...
            switch (menuChoice) {
                ...
                case 3 -> forgotPasswordFlow(crs, scanner);
    ...
    public static void adminMenu(CourseRegistrationSystem crs, Scanner scanner, User currentUser) {
        ...
        do {
            try {
                ...
                System.out.println("| 4. Admin Related Options         |");
    ...
    private static void forgotPasswordFlow(CourseRegistrationSystem crs, Scanner scanner) {
        ...
        if (role.equals("student")) {
            ...
            if (student == null || !email.equalsIgnoreCase(student.getEmail())) {
                System.out.println("Student ID and email do not match our records.");
                return;
            }
            ...
            username = crs.getUsernameByEmail(email);
            if (username == null) {
                System.out.println("No admin account found with this email.");
                return;
            }

        } else {
            System.out.println("Invalid role. Please enter 'student' or 'admin'.");
            return;
        }

        // Generate and send verification code
        String code = String.valueOf(100000 + new Random().nextInt(900000));
        verificationCodes.put(email, new VerificationEntry(code));
        ...
        if (entry == null || !entry.code.equals(input)) {
            System.out.println("Invalid code. Password reset failed.");
            return;
        }

        long now = System.currentTimeMillis();
        if (now - entry.timestamp > 60_000) { // 60,000 milliseconds = 1 minute
            System.out.println("Verification code expired. Please request a new one.");
            verificationCodes.remove(email); // remove expired entry
            return;
        }
    ...
    public static void addStudent(CourseRegistrationSystem crs, Scanner scanner) {
        ...
        String email;
        while (true) {
            System.out.print("Enter student email (or type 'exit' to cancel): ");
            email = scanner.nextLine().trim();
    ...
    public static void viewAdminDetails(CourseRegistrationSystem crs, Scanner scanner) {
        System.out.print("Enter admin username to view: ");
        String username = scanner.nextLine();
        crs.viewUserDetails(username, "admin");
    }
}
